...
def complex_dropout(input, dropout, is_training=False):
    if is_training:
...
class CompEncoderBlock(nn.Module):
    ...
    def __init__(self, configs, extended=True):
        ...
        self.is_emb = True
        self.ori_len = int((configs.seq_len + configs.pred_len) / 2) + 1 - configs.low_cut
        self.tar_len = int((configs.seq_len + configs.pred_len) / 2) + 1 - configs.low_cut
