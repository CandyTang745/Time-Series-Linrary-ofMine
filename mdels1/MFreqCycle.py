...
class Model(nn.Module):
    ...
    def forward(self, x, x_mark_enc=None, x_dec=None, x_mark_dec=None, cycle_index=None ):
        ...
        for i in range(self.Cycle_num-1):
            ...
            # x_cycle=self.cycleQueue[i](cycle_index[:,i],x_down.size(1)) #作者实现错误，cycle_index[batch]
            x_cycle=self.cycleQueue[i](cycle_index % self.every_Cycle[i],x_down.size(1)) #最小改动使模型能跑通

            x_down=x_down-x_cycle
            y_down=self.seg_layer[i](x_down.permute(0, 2, 1)).permute(0, 2, 1)
            y_period=self.cycleQueue[i]((((cycle_index % self.every_Cycle[i])+self.recent_num[i])//self.Cycle_stride[i])%(self.every_Cycle[i]//self.Cycle_stride[i]),(self.pred_len-self.Cycle_window[i])//self.Cycle_stride[i]+1)
