'''
FreqCycle: A Multi-Scale Time-Frequency Analysis Method for Time Series
Forecasting
框架 FreqCycle，该框架整合了两大模块：
（1）滤波增强周期预测（FECF）模块，通过在时域中显式学习共享周期模式以提取低频特征；
（2）分段频域模式学习（SFPL）模块，借助可学习滤波器与自适应加权提升中高频能量占比。
'''
import torch
